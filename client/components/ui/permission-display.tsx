                      ...
                      <div className="mt-3 pt-3 border-t">
                        ...
                        {resourcePermissions.map((permission, index) =>
                          permission.conditions?.map((condition, condIndex) => (
                            ...
                            >
                              {condition.type}:{" "}
                              {condition.value || t("rbac.common.applies")}
                            </Badge>
                          )),
                        )}
                      </div>
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse border border-border">
          <thead>
            <tr>
              <th className="border border-border p-2 text-left bg-muted">
                {t("rbac.common.resource_header")}
              </th>
              {Array.from(selectedRoles).map((role) => (
                <th
                  key={role}
                  className="border border-border p-2 text-center bg-muted capitalize"
                >
                  {t(`roles.labels.${role}`)}
                </th>
              ))}
